...
class Actor(nn.Module):
    """
    Standard Actor (No Adaptive Limits).
    """
...
# -----------------------------------------------------------
# 3. RACER AGENT (Simplified: Quantile + PER, No Adaptive Limits)
# -----------------------------------------------------------
...
class RACER_Quantile_PER:
    def __init__(self, env, config):
        ...
        # Networks
        self.actor = Actor(self.obs_shape, self.beam_dim, self.action_dim, 
    ...
    def get_action(self, state, explore=True):
        ...
        # Check for 4 dims (Channels, Depth, Height, Width) -> Add Batch
        if dose.dim() == 4: 
    ...
    def update(self, per_beta=0.4):
        ...
        # ----------------------------
        # CRITIC UPDATE (Standard Quantile Regression)
        # ----------------------------
        with torch.no_grad():
            ...
            # SIMPLIFICATION: No TQC Truncation. Use full pooled distribution.
            target_pooled = target_q.view(self.batch_size, -1)
            
            # Standard distributional Bellman update
            target_Z = rew + (1 - done) * self.gamma * target_pooled

        current_q = self.critic(state, act)
        critic_loss = 0.0
        
        # Expand target for pairwise Huber loss
        target_Z_expanded = target_Z.unsqueeze(1) # [Batch, 1, Total_Atoms]
        
        for i in range(self.critic.ensemble_size):
            # Pred: [Batch, N_Quantiles, 1]
        ...
        # Update PER Priorities using Mean Absolute TD Error
        with torch.no_grad():
...
if __name__ == "__main__":
    ...
    print("--- STARTING SIMPLIFIED RACER TRAINING ---")
    ...
    for ep in range(TOTAL_EPISODES):
        ...
        while not done:
            ...
            # PER Beta Annealing (0.4 -> 1.0 over time)
        ...
        if ep % 10 == 0:
            ...
            print(f"Ep {ep} | Reward: {episode_reward:.2f} | Avg100: {avg_score:.2f} | CVaR: {cvar_str}")
